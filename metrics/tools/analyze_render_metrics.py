...
from config.settings import RENDER_METRICS_FILE
...
def summarize(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    ...
    for model_name, items in sorted(grouped.items()):
        timings = [
            value
            for value in (_to_float(item.get("click_to_first_frame_ms", "")) for item in items)
        ...
        summary.append(
            {
                ...
                "file_size_bytes": items[-1].get("file_size_bytes", ""),
...
def main() -> int:
    rows = load_rows(RENDER_METRICS_FILE)
    if not rows:
        print(f"No metrics found: {RENDER_METRICS_FILE}")
        return 0

    print(f"Metrics file: {RENDER_METRICS_FILE}")
