...
with col_left:
    ...
    if user_input:
        ...
        if cif_name:
            ...
            # 如果没找到文件（比如只是闲聊，没调工具），保持 None 或清空
            pass
...
with col_right:
    ...
    if st.session_state.tasks:
        ...
        st.markdown(
            ...
            <div class="card">
                <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.8rem;">
                    ...
                    <div style="font-size: 0.9rem; font-weight: 600;">Agent 执行步骤</div>
                </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        decomposition_tasks = [
            {
                "title": "意图识别",
                "text": "识别用户查询的材料实体 (如 LiFePO4)",
                "active": True,
            },
            {
                "title": "API 调用",
                "text": "通过 Materials Project 获取结构数据",
                "active": True,
            },
            {
                "title": "结构解析",
                "text": "解析 CIF 文件生成 Lattice/XRD 数据",
                "active": True,
            },
        ]
    ...
    if st.session_state.viz_data is not None:
        ...
        st.caption(f"当前加载文件: {st.session_state.viz_data['filename']}")
