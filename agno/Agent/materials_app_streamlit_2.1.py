...
def generate_real_tasks():
    """
    核心逻辑改造：基于 Session State 中的真实产物动态生成任务状态。
    遵循 "Source of Truth" 原则。
    """
    tasks = []

    # ================== 任务 1: 意图识别 ==================
    # 产物锚点: st.session_state.last_question
    ...
    # ================== 任务 2: 数据获取 ==================
    # 产物锚点: st.session_state.viz_data (如果不为空，说明文件已落地且读取成功)
    viz_data = st.session_state.get("viz_data")
    has_data = viz_data is not None

    # 如果任务1完成了但任务2没数据，可能是失败了或者正在运行（但在Streamlit同步流中通常意味着未完成）
    t2_status = "done" if has_data else "pending"

    tasks.append({
        "id": "T2",
        "label": "Materials Project 接口调用与数据落地",
        "status": t2_status,
        "progress": "100%" if has_data else "0%"
    })

    # ================== 任务 3: Agent 推理 ==================
    # 产物锚点: st.session_state.messages 中最后一条是 Assistant
    ...
    tasks.append({
        ...
        "status": "done" if has_reply else "pending",
        "progress": "100%" if has_reply else "0%"
    })

    # ================== 任务 4: 可视化生成 ==================
    # 产物锚点: viz_data 中的 xrd_df 是否有数据
    t4_status = "pending"
    t4_progress = "0%"

    if has_data:
        # 只要有 viz_data，至少通过了已获取结构这一步
        if not viz_data['xrd_df'].empty:
            # XRD 算出来了 -> 完美
            t4_status = "done"
            t4_progress = "100%"
        else:
            # 只有 Lattice，没有 XRD -> 进度 50%
            t4_status = "running"  # 或者用 pending，这里用 running 表示"进行了一半"
            t4_progress = "50%"
